...
import { PulseDistributor } from '../../types/ethers-contracts/PulseDistributor';
import { PulseDistributor__factory } from '../../types/ethers-contracts/factories/PulseDistributor__factory';
...
async function deploy() {
    ...
    const tokenAddress = mainnet ? address.mainnet.tokens.pulse : address.testnet.tokens.tefi;
    const distAddress = mainnet ? address.mainnet.ditributors.pulse : address.testnet.distributors.tefi;

    const factory: PulseDistributor__factory = new PulseDistributor__factory(deployer);
    let dist: PulseDistributor = factory.attach(distAddress).connect(deployer);
    if ("Redeploy" && true) {
        dist = await factory.deploy(tokenAddress, routerAddress, swapAddress, "0x32f1C25148DeCbdBe69E1cc2F87E0237BC34b700");
    }

    console.log('PulseDistributor: ', dist.address);
