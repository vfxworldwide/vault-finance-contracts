...
import { PriceCalculator } from '../types/ethers-contracts/PriceCalculator';
import { PriceCalculator__factory } from '../types/ethers-contracts/factories/PriceCalculator__factory';
...
async function deploy() {
    ...
    const priceAddress = mainnet ? address.mainnet.price : address.testnet.price;

    const factory: PriceCalculator__factory = new PriceCalculator__factory(deployer);
    let price: PriceCalculator = factory.attach(priceAddress).connect(deployer);
    if ("Redeploy" && true) {
        price = await factory.deploy();
    }

    console.log('PriceCalculator: ', price.address);
